<?php
/**
 * PayPal IPN通知用のipn_main_handler.phpコールバックハンドラ
 *
...
// チェックアウトのためカートを検証
  ...
  // カートの内容に違反があると判断した場合、チェックアウトを防止するためのチェック
  ...
  // 最後のパスからカートの内容が変更された場合リセット
  if (isset($_SESSION['cart']->cartID)) {
    if (isset($_SESSION['cartID'])) {  // これは、顧客がcheckout_shippingページにアクセスした場合にのみ設定されます。 ECショートカットボタンで起動すると設定されませんので、その場合はリダイレクトしないでください。
...
Processing...
  ...
  /**
   * PayPalから戻って来る余分なURLでエンコードされたPOSTデータの奇妙なケースを検出する
   */
  ...
  /**
   * paypal固有のapplication_topパラメータを設定する
   */
  ...
  if (  (defined('MODULE_PAYMENT_PAYPALWPP_DEBUGGING') && strstr(MODULE_PAYMENT_PAYPALWPP_DEBUGGING, 'Log')) ||
    ...
    @ini_set('display_errors', 0); // 画面/ブラウザ/クライアントにエラーを出力しない（ログファイルのみ）
  ...
  /**
   * 確認をPayPalにpost-backし、その後の使用のために結果を抽出する
   */
  $info  = ipn_postback();
  $new_status = 1;
  ...
  /**
   * トランザクションを検証する - 電子メールアドレス、一致するtxnレコード, など
   */
  ...
  // レコードを挿入する必要があるかどうかを判断するために使用されます。すなわち、元のecheckの通知は失敗しましたが、今度はクリアしたので、親レコードが必要です:
  ...
  switch ($txn_type) {
    ...
    case 'pending-unilateral':
      // マーチャントのPayPalアカウントがまだ有効でないため、この注文を処理することはできません
    ...
    case (($txn_type == 'express_checkout' || $isECtransaction) && !strstr($txn_type, 'cleared') && $parentLookup != 'parent'):
      if ($_POST['payment_status'] == 'Completed') {
        // これはエクスプレスチェックアウト取引です -  IPNは必要ないかもしれません
    ...
    case 'unique':
      /**
       * PayPalテーブルからIPNセッションを削除する - ハウスキーピング
       */
      ...
      if (!strstr($txn_type, 'denied') && !strstr($txn_type, 'failed') && !strstr($txn_type, 'voided')) {
        ...
        /** ノーティファイヤクラスで使用するセールストラッキングデータを準備 **/
    ...
    case 'express-checkout-cleared':
      ...
      break;
    default:
      // 見つかった結果を理解できません。したがって、ロギングとアボート。
